#!/bin/sh
# The official bitchat-tui installer
...
main() {
    ...
    local _bin_name
    case "${_arch}" in
    *windows*) _bin_name="bitchat-tui.exe" ;;
    *) _bin_name="bitchat-tui" ;;
    esac
    ...
    local _package
    _package="$(ensure download_bitchat_tui "${_arch}")"
    ...
    *)
    ...
    ensure try_sudo cp -- "${_bin_name}" "${BIN_DIR}/bitchat-tui"
    ensure try_sudo chmod +x "${BIN_DIR}/bitchat-tui"
    echo "Installed bitchat-tui to ${BIN_DIR}"
    ...
    echo ""
    echo "bitchat-tui is installed!"
    if ! echo ":${PATH}:" | grep -Fq ":${BIN_DIR}:"; then
        echo "Note: ${BIN_DIR} is not on your \$PATH. bitchat-tui will not work unless it is added to \$PATH."
    fi
}
...
usage() {
    ...
    echo "\
${_text_heading}bitchat-tui installer${_text_reset}
Vaibhav Mattoo <vaibhavmattoo1@gmail.com>
https://github.com/vaibhav-mattoo/bitchat-tui

Fetches and installs bitchat-tui. If bitchat-tui is already installed, it will be updated to the latest version.
...
download_bitchat_tui() {
    ...
    local _releases_url="https://api.github.com/repos/vaibhav-mattoo/bitchat-tui/releases/latest"
    local _releases
    case "${_dld}" in
    ...
    *) err "unsupported downloader: ${_dld}" ;;
    ...
    (echo "${_releases}" | grep -q 'API rate limit exceeded') &&
        err "you have exceeded GitHub's API rate limit. Please try again later, or use a different installation method: https://github.com/vaibhav-mattoo/bitchat-tui/#installation"

    local _package_url
    _package_url="$(echo "${_releases}" | grep "browser_download_url" | cut -d '"' -f 4 | grep -- "${_arch}")" ||
        err "bitchat-tui has not yet been packaged for your architecture (${_arch}), please file an issue: https://github.com/vaibhav-mattoo/bitchat-tui/issues"
    ...
    *) err "unsupported package format: ${_package_url}" ;;
    ...
    local _package="bitchat-tui.${_ext}"
    case "${_dld}" in
